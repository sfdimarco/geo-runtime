...
export async function render({
  ...
  if (win) { [X0, Y0, X1, Y1] = win; }
  else {                                  // one shared box, so a frame cannot silently rescale
    ...
    const pad = 0.08 * Math.max(hi[0] - lo[0], hi[1] - lo[1]);
  ...
  for (let f = 0; f < T.length; f++) {
    ...
    for (let i = 0; i + 2 < ix.length; i += 3) {
      ...
      const p = T3.map(v => { const [sx, sy] = sxy(m[v*b.stride], m[v*b.stride+1]); return [sx, sy, m[v*b.stride+2]]; });
      const ar = (p[1][0]-p[0][0])*(p[2][1]-p[0][1]) - (p[2][0]-p[0][0])*(p[1][1]-p[0][1]);
      if (ar === 0) continue;
      let n = [0, 0, 0];
      for (const v of T3) { const o = v*b.stride+5; n[0]+=m[o]; n[1]+=m[o+1]; n[2]+=m[o+2]; }
